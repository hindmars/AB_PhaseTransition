...
def fphi_extrame(lambda_bar, delta_pT, lambda_pT):
    ...
    # extreme point
    preCoef = delta_pT/(2*lambda_pT)

    phin = preCoef*(1-(1 - (8/9) * lambda_bar)**(1/2))
    phip = preCoef*(1+(1 - (8/9) * lambda_bar)**(1/2))
    
    # two extreme value of fphi
    f1 = (4/3) * lambda_bar
    ...
    return fphi_n, fphi_p, phin, phip
...
# calculate the 0 temperature phi0, Tcp
alpha, beta1, beta2, beta3, beta4, beta5, betaA, betaB, DeltaA, DeltaB, phi0, Tcp, xitp, xi0p, t = SC_beta_gaps.calculate_SC_beta_gaps_etc(pressure, 0.0*Kelvin)
print("\n zero-temperature phi0 is : ", phi0)
Phi0 = phi0

stepT = 0.1*Tcp 
Temperature = np.arange(0.0*(10**-3), 1*Tcp, stepT) #Kelvin
...
# phi array
stepPhi = 0.01*Phi0
phi = np.arange(0.0, 1.2*Phi0, stepPhi)
lengthPhi = len(phi)

# phi_n array, phi_p array, fphi_n array, fphi_p array
array_phi_n = []; array_fphi_n = []
array_phi_p = []; array_fphi_p = []
...
# saving array for fphi
array_fphi = np.zeros((lengthT, lengthPhi))

for iT in range(0, lengthT, 1):
        
    indexT = iT
    print('\n\n indexT is',indexT, ' Temperature is ', Temperature[indexT])
    ...
    if t >= 1:
       ...
       # array_ratio_fphin_fphip[indexP,indexT] = np.nan
        ...
        # calculate the lambda_bar 
        lambda_bar = (9/2)*lambda_pT*(M2/(delta_pT*delta_pT))
        array_lambda_bar.append(lambda_bar)
        ...
        fphi_n, fphi_p, phin, phip = fphi_extrame(lambda_bar, delta_pT, lambda_pT)

        array_phi_n.append(phin); array_fphi_n.append(fphi_n)
        array_phi_p.append(phip); array_fphi_p.append(fphi_p)

        print("\n\n lambda_bar is : ", lambda_bar, " fphi_n is: ", fphi_n, " fphi_p is: ", fphi_p, " phin is: ", phin, " phip is: ", phip, " \n\n ")
        ...
        ###################################################################
        ####           calculate fphi over phi matrix                 #####
        ###################################################################

        for iphi in range(0, lengthPhi, 1):

            fphi = (1/2)*M2*(phi[iphi])**(2) - (1/3)*delta_pT*(phi[iphi])**(3) + (1/4)*lambda_pT*(phi[iphi])**(4)

            array_fphi[indexT, iphi] = fphi
            
              
    print("\n fphi now is : ", array_fphi[indexT,:])
...
# for iT in range(0, 1, 1):
    print("\n\n scaled temperature is :",Temperature[iT]/Tcp, " lambda_bar is :", array_lambda_bar[iT] )
    
    plot1.plot(phi, array_fphi[iT,:])

# scatter plot of the fphi_n and fphi_p
plot1.scatter(array_phi_n, array_fphi_n, color='orange', marker='h')
plot1.scatter(array_phi_p, array_fphi_p, color='blue', marker='d')

plot1.xlabel(r'$\phi/J$');plot1.ylabel(r'$f(\phi)/J. m^{-3}$')
plot1.ylim([-1.2, 8.0]);plot1.xlim([0.0, 1.2*Phi0])

# add legend
plot1.legend([r't = 0.0, $\bar{\lambda} = 0.9302$',r't = 0.1, $\bar{\lambda} = 0.9397$',r't = 0.2, $\bar{\lambda} = 0.9490$',r't = 0.3, $\bar{\lambda} = 0.9581$',r't = 0.4, $\bar{\lambda} = 0.9671$',r't = 0.5, $\bar{\lambda} = 0.9359$', r't = 0.6, $\bar{\lambda} = 0.9845$',r't = 0.7, $\bar{\lambda} = 0.9930$',r't = 0.8, $\bar{\lambda} = 1.0012$',r't = 0.9, $\bar{\lambda} = 1.0091$',r'$f(\phi_{-})$',r'$f(\phi_{+})$'],loc = 'upper left')
plot1.title(r'$f(\phi)$ evolution with $t$, pressure = $32$bar')

plot1.savefig('fphi_32bar_plot_with_fphin_fphip_and_lambdaBar.pdf')
