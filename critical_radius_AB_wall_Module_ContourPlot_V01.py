...
Pressure = np.arange(21.2,30.1,0.1) # bar
...
##############################################################################
#####              calculate the temperature shift                       #####
##############################################################################

T_shift_array = np.ones(T_array.shape)
for iP in range(0, len(Pressure), 1):
  T_shift_array[iP, :] = (TAB_Greywall[iP] - TAB_RWS[iP])*T_shift_array[iP, :]

print("\n T_shift_array looks like\n", T_shift_array)
...
for p in Pressure:
  if p < 21.5:
   tensionAB_p = np.append(tensionAB_p, 0.731)
...
leg2 = ax.legend([TCPlts,TABPlts,sIC,sHEC],[r"$T_{c}^{PLTS}$", r"$T_{AB}^{PLTS}$", "p-T IC", "p-T HEC"],  fontsize=15.0, loc='lower right')
# ax.add_artist(leg1)
