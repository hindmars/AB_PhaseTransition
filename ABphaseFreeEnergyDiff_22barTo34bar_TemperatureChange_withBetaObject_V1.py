...
stepT = 0.02*(10**-3) 
Temperature = np.arange(1.2*(10**-3), 2.6*(10**-3), stepT) #Kelvin
...
for iP in range(0, lengthPressure, 1):
    #indexT = math.floor(T/stepT)
    indexP = iP
    plot1.plot(Temperature*1000, DiffFABGLScaled[indexP,:], '-.'); plot1.ylabel(r'$(f^{A}_{GL}-f^{B}_{GL})$/$|f^{B}_{GL}|$'); plot1.xlabel(r'$T$/mK')

plot1.savefig('FreeEnergyDiff_Scaled_21bar_To_33bar.pdf')    
plot1.show()    
plot1.clf()
plot1.cla()
plot1.close()    
...
plot1.plot(Temperature*1000, DiffFABGLScaled[0,:], '-.', Temperature*1000, DiffFABGLScaled[1,:], '-.', Temperature*1000, DiffFABGLScaled[2,:], '-.', Temperature*1000, DiffFABGLScaled[3,:], '-.', Temperature*1000, DiffFABGLScaled[4,:], '-.', Temperature*1000, DiffFABGLScaled[5,:], '-.', Temperature*1000, DiffFABGLScaled[6,:], '-.');
...
plot1.ylabel(r'$(f^{A}_{GL}-f^{B}_{GL})$/$|f^{B}_{GL}|$'); plot1.xlabel(r'$T$/mK')
