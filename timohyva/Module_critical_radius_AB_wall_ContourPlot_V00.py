...
m3 = 3.016293*u #mass of helium3 atom
  ...
  ##############################################################################
  #####                module function starts from here                   ######
  ##############################################################################

def plot_contour(fudge_Array):
  ...
  Pressure = np.arange(20.0,30.0,0.1) # bar
...
# #############################################################################
  ...
  for iP in range(0, len(Pressure), 1):
      ...
      for it in range(0, len(Temperature), 1):
         ...
         if (p < 21.22) and (t <= 1.):
          ...
          fB = SCC.alpha_td(p, T)/SCC.betaB_td(p, T, f_arr)

          fBA = (-SCC.alpha_td(p,T)/SCC.betaB_td(p, T, f_arr)) + (SCC.alpha_td(p,T)/SCC.betaA_td(p, T, f_arr))
        ...
        # A to B or B to A
           elif (t == SCC.tAB_RWSfarr(p, f_arr)):
  ...
  # ax.set_title(r"contour plot of $|(\sigma_{AB}(p)/f_{AB}(p,T))|/{\mu}m$", fontsize=14.0,)
  ax.set_title(r"$q_1={:.4g},q_2={:.4g},q_3={:.4g},q_4={:.4g},q_5={:.4g}$".format(*f_arr), fontsize=14.0,)
  ax.set_xlim([1.8, 2.4])
  ax.set_ylim([20., 30.])
  ax.grid(True)

  # fig.savefig("Rc_contour_RWS_GreywelltoPltsTC_AToBBToA4.pdf")
