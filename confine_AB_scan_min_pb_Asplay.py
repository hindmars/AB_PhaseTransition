...
for m, p in enumerate(p_arr):
    ...
    for n, t in enumerate(t_arr[t_arr < 1]):
        ...
        Apg_conf_AA_list[m][n], sigma_bw_AA_arr[m,n], sigma_tot_AA_arr[m,n] = c.get_confine(t, p, w, "Ay", "Ay", 
                                                    bcs = bcs, T_list=[h.z3, h.T_xy/2, h.z3], 
...
for m, p in enumerate(p_arr):
    ...
    for n, t in enumerate(t_arr[t_arr < 1]):
        if Apg_conf_AA_list[m][n] is not None:
            print(Apg_conf_AA_list[m][n][1].args, sigma_bw_AA_arr[m,n], sigma_tot_AA_arr[m,n])

#%%

confine_sol_data_min_1100_AA = (Apg_conf_AA_list, sigma_bw_AA_arr, sigma_tot_AA_arr, T_arr, p_arr)

with open(f'confine_sol_data_min_1100_AA{id:}.pickle', 'wb') as f:
