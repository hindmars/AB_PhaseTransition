...
def do_one_plot(ax, sc_type, p, ls_mk, lbls=None):
    ...
    beta_345 = (h.beta_norm(1, p, 3) + h.beta_norm(1, p, 4) + h.beta_norm(1, p, 5)) /h.beta_const
    beta_plot(ax, p, beta_345, ls_mk, 'k', sc_type)
...
ax.set_title(r'$\beta^{\rm sc}_{345}/\beta_0$')

fig.tight_layout()


if savefig:
    fig.savefig('beta_345_compare.pdf')
