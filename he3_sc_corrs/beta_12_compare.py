...
lbls_12 = r'$\beta^{{\rm sc}}_{{ {} }}/\beta_0$'.format('12') 
...
def do_one_plot(ax, sc_type, p, ls_mk, lbls=None):
    ...
    beta_12 = (h.beta_norm(1, p, 1) + h.beta_norm(1, p, 2)) /h.beta_const
    beta_plot(ax, p, beta_12, ls_mk, 'k', sc_type)
...
ax.set_title(r'$\beta^{\rm sc}_{12}/\beta_0$')

fig.tight_layout()


if savefig:
    fig.savefig('beta_12_compare.pdf')
