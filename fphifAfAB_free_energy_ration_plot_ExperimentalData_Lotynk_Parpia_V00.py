...
import Module_SCC_bB_V00 as SCCB
import Module_fphi_AtoB as fphi_bar_AtoB
...
for iP in range(0, len(pressure_Lotynk), 1):
    ...
    if t >= 1:
      ...
      fphi_n_Lotynk = np.append(fphi_n_Lotynk, np.nan)
      ...
      fA_Lotynk = np.append(fA_Lotynk, -(((SCCB.alpha_bar(p, T))**2)/(4.*SCCB.betaA_bar(p, T))))

      (fphi_n_bar, fphi_p_bar) = fphi_bar_AtoB.fphi_ebar(p, T)
      fphi_n_Lotynk = np.append(fphi_n_Lotynk, fphi_n_bar)
     
      fBA_Lotynk = np.append(fBA_Lotynk, (-(((SCCB.alpha_bar(p, T))**2)/(4.*SCCB.betaB_bar(p, T)))) - (-(((SCCB.alpha_bar(p, T))**2)/(4.*SCCB.betaA_bar(p, T)))))

      print(" \n Lotynk fA is ", fA_Lotynk[iP], " Lotynk fBA is ", fBA_Lotynk[iP], " Lotynk fphi_n is ", fphi_n_Lotynk[iP])

# ratio fA/fBA
ratio_Lotynk = (fA_Lotynk + fphi_n_Lotynk)/fBA_Lotynk; print("\n (fA + fphi)/fBA looks like : ", ratio_Lotynk)
...
fphi_n_IC_Parpia = np.array([])
fBA_IC_Parpia = np.array([])

fA_HEC_Parpia = np.array([])
fphi_n_HEC_Parpia = np.array([])
...
for iP in range(0, len(Parpia.pressure), 1):
    ...
    if t_IC >= 1:
      ...
      fphi_n_IC_Parpia = np.append(fphi_n_IC_Parpia, np.nan)
      ...
      fA_IC_Parpia = np.append(fA_IC_Parpia, -(((SCCB.alpha_bar(p, TIC))**2)/(4.*SCCB.betaA_bar(p, TIC))))

      (fphi_n_bar, fphi_p_bar) = fphi_bar_AtoB.fphi_ebar(p, TIC)
      fphi_n_IC_Parpia = np.append(fphi_n_IC_Parpia, fphi_n_bar)
     
      fBA_IC_Parpia = np.append(fBA_IC_Parpia, (-(((SCCB.alpha_bar(p, TIC))**2)/(4.*SCCB.betaB_bar(p, TIC)))) - (-(((SCCB.alpha_bar(p, TIC))**2)/(4.*SCCB.betaA_bar(p, TIC)))))

      print(" \n Parpia fA_IC is ", fA_IC_Parpia[iP], " Parpia fBA_IC is ", fBA_IC_Parpia[iP], " Parpia fphi_n_bar_IC is ", fBA_IC_Parpia[iP])
    ...
    if t_HEC >= 1:
      ...
      fphi_n_HEC_Parpia = np.append(fphi_n_HEC_Parpia, np.nan)
      ...
      fA_HEC_Parpia = np.append(fA_HEC_Parpia, -(((SCCB.alpha_bar(p, THEC))**2)/(4.*SCCB.betaA_bar(p, THEC))))

      (fphi_n_bar, fphi_p_bar) = fphi_bar_AtoB.fphi_ebar(p, THEC)
      fphi_n_HEC_Parpia = np.append(fphi_n_HEC_Parpia, fphi_n_bar)
          
      fBA_HEC_Parpia = np.append(fBA_HEC_Parpia, (-(((SCCB.alpha_bar(p, THEC))**2)/(4.*SCCB.betaB_bar(p, THEC)))) - (-(((SCCB.alpha_bar(p, THEC))**2)/(4.*SCCB.betaA_bar(p, THEC)))))

      print(" \n Parpia fA_IC is ", fA_HEC_Parpia[iP], " Parpia fBA_IC is ", fBA_HEC_Parpia[iP], " Parpia fphi_n_bar_HEC is ", fphi_n_HEC_Parpia[iP])  

# ratio fA/fBA
ratio_IC_Parpia = (fA_IC_Parpia + fphi_n_IC_Parpia)/fBA_IC_Parpia; print("\n fA/fBA IC looks like : ", ratio_IC_Parpia)
ratio_HEC_Parpia = (fA_HEC_Parpia + fphi_n_HEC_Parpia)/fBA_HEC_Parpia; print("\n fA/fBA HEC looks like : ", ratio_HEC_Parpia)
...
ax.set_ylabel(r"$\frac{f_{A} - f(\phi_{-})}{f_{B}-f_{A}}$")
ax.grid(True)

fig.savefig("fAfphinFBA_ratio.pdf")
plt.show()
