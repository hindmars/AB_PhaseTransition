...
stepT = 0.0001*(10**-3) 
Temperature = np.arange(1.8*(10**-3), 2.50*(10**-3), stepT) #Kelvin

stepPressure = 0.01*bar
# pressure = np.arange(20.0, 33.9*bar, stepPressure)
pressure = np.arange(19.0, 22.6*bar, stepPressure)
...
LeggttEstimate_Rc_DiffFABGLScaled = np.zeros((lengthPressure,lengthT)) # save data of the thin-wall evaluation of R_{c} with experiment tension \sigma = 0.7 \xi f_{B}
...
#       fBGLRed = alphaRed*3*(delta**2) + 3*beta12345Red*(delta**4)
           ...
           TempData = (fAGLRed - fBGLRed)/abs(fBGLRed)
           if TempData > 0:
              LeggttEstimate_Rc_DiffFABGLScaled[indexP,indexT] = (2*0.7)/TempData
              LeggttEstimate_Rc_DiffFABGLScaled_logarithim[indexP,indexT] = math.log((2*0.7)/TempData)
              print(" Fucking is ", LeggttEstimate_Rc_DiffFABGLScaled[indexP,indexT])

           else:

              print(" bro, you get negative logarithm, save a np.nan. ")

              LeggttEstimate_Rc_DiffFABGLScaled[indexP,indexT] = np.nan
              LeggttEstimate_Rc_DiffFABGLScaled_logarithim[indexP,indexT] = np.nan  
    ...
    print('Leggtt extimated Rc is: ', LeggttEstimate_Rc_DiffFABGLScaled[indexP,:])
#    plo
...
plot1.contourf(X*(10**3), Y, LeggttEstimate_Rc_DiffFABGLScaled, cmap=cm.PuBu_r, levels=Levels);plot1.ylabel(r'$p/bar$'); plot1.xlabel(r'$T$/mK');
...
# Cplot = plot1.contour(Temperature*1000, pressure, LeggttEstimate_Rc_DiffFABGLScaled, colors='black');plot1.clabel(Cplot, inline=True, fontsize=8.5, colors='r')
...
# # DensityPlot = plot1.pcolormesh(Temperature*1000, pressure, LeggttEstimate_Rc_DiffFABGLScaled_logarithim,
# #                                norm=matplotlib.colors.LogNorm(vmin=LeggttEstimate_Rc_DiffFABGLScaled_logarithim.min(), vmax=LeggttEstimate_Rc_DiffFABGLScaled_logarithim.max()));
...
LLLLL = [1*10**0, 3*10**0, 5*10**0, 7*10**0, 9*10**0, 10**1, 3*10**1, 5*10**1, 7*10**1, 9*10**1, 10**2, 3*10**2, 5*10**2, 7*10**2, 9*10**2, 10**3] 
X, Y = np.meshgrid(Temperature, pressure)
# fig, ax = plot1.subplots()
cs = plot1.contourf(X*(10**3), Y, LeggttEstimate_Rc_DiffFABGLScaled, locator=ticker.LogLocator(), cmap=cm.PuBu_r, levels=LLLLL);plot1.ylabel(r'$p/bar$'); plot1.xlabel(r'$T$/mK');
# cs.collections[0].set_label(r'$2 {R^{thin-wall}_{c}}/{\xi}={\sigma^{experiment}_{AB}}/{\Delta}f_{AB}{\xi}$')
# plot1.legend(loc='lower right')
plot1.title(r'${R^{thin-wall}_{c}}/{\xi}={2{\sigma}^{experiment}_{AB}}/{\Delta}f_{AB}{\xi}$')
plot1.colorbar(cs)
...
plot1.savefig('Distribution_of_Rc_Leggtt_Estimateion_LogarithmCoordinate_Logarithim_Contour_PolyCritical_withExperimentalData_T18.pdf');
# plot1.show()
