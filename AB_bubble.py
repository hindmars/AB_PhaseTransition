...
def get_and_plot_wall(t,p):
    ...
    A, pot, gr = hw.krylov_bubble(t,p, gr_pars=(500,50), dim=3)
    ...
    ax[0].set_title(r'AB boundary, p={:.1f} bar: $T/T_{{\rm AB}} = {:.1f}$'.format(p, t/h.t_AB(p)))
...
S_AB = hw.energy(*Apg_kry)[0] * h.f_scale(p)*h.xi(0,p)**3/(h.kB * t * h.Tc_mK(p)*1e-3)

print('Energy/kBT:', S_AB)
