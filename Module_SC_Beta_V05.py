...
################################################################################
##'''     sepcial version of xiGL_OC, Tc digistzed from Parpia
#'
def xiGL_OC_Parpia(p, Tcp, t):
   # p,T dependent GL coherent length, nanometer
   Kelvin = 1;J = 1.; s = 1.;
   hbar = 1.054571817*(10**(-34))*J*s;kb = 1.380649*(10**(-23))*J*(Kelvin**(-1))
...
# AB interface tension in unit of \xiGH_OC * fB, data sheet from Mark's code
def sigma_AB(t): BetaObject.sigma_function(SC.t_scale,SC.Sigma_Scho,t); return BetaObject.sigma
