...
bcs_gap_A_phase_arr = np.array([float(scopt.fsolve(bcs.gap_eqn_A_phase, 2, args=(t,))) for t in t_arr])


#%% Get A phase free energy

f_A_t_list = []

for t in t_arr:
    
    f_A_arr = np.zeros_like(D_arr)
    
    for n, D in enumerate(D_arr):
        f_A_arr[n] = bcs.free_energy_A_phase(D, t)
    
    f_A_t_list.append(f_A_arr)

f_A_t_arr = np.array(f_A_t_list)
...
ax1.plot(t_arr, bcs_gap_A_phase_arr**2)
...
ax1.set_title('BCS gap squared A phase')
...
for n in range(7,len(t_arr),8):
    ...
    popt, copt = scopt.curve_fit(bcs.poly_in_squares, (D_arr)[f_A_arr<f_lim], f_A_arr[f_A_arr<f_lim], p0=(-t, .1, .1))
    
    line = ax3.plot(D_arr, f_A_t_arr[n,:] - f_A_t_arr[n,0])
    ...
    print(popt[0]/2, popt[1]/4, popt[2]/6 )
...
ax3.set_title('BCS A-phase free energy (solid), fit to $a_0x^2 + a_1x^4 + a_2x^6$ (dash)' 
              + '\n'
              + r'Fitted $\beta_{{245}} = {:.5f}$  '.format(popt[1]/4)
...
np.savez('bcs_gap_A_phase.npz', t=t_arr, bcs_gap_A_phase=bcs_gap_A_phase_arr)

np.savez('free_energy_A_phase.npz', t=t_arr, delta_A=D_arr, free_energy_A_phase=f_A_t_arr)
