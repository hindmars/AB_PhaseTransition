...
# This script is used for calculating the gap energies,
# densities of condesated energies of A and B phases
# and their difference as well.
...
EnergyGap_A = np.zeros((lengthPressure,lengthT))

EnergyGap_B = np.zeros((lengthPressure,lengthT))
...
for iP in range(0, lengthPressure, 1):
    ...
    for iT in range(0, lengthT, 1):
        ...
        if t >= 1:
           ...
           EnergyGap_A[indexP,indexT] = np.nan

           EnergyGap_B[indexP,indexT] = np.nan
...
#       fBGLRed = alphaRed*3*(delta**2) + 3*beta12345Red*(delta**4)
           ...
           # save energy gap of A
           EnergyGap_A[indexP,indexT] = deltaA

           # save energy gap of B
           EnergyGap_B[indexP,indexT] = deltaB
           ...
           print('gap A in SI unit is:', deltaA)
           print('Gap B in SI unit is:', deltaB)
...
plot1.savefig('Distribution_of_GL_CoherentLength_Contour_SCModule_versionIII.pdf');
...
####################################################################
# contour plot of temperature-pressure dependent energy density of A
####################################################################
...
X, Y = np.meshgrid(Temperature, pressure)
# cs1 = plot1.contourf(X*(10**3), Y, TempretureDependent_GL_CoherentLength*(10**9), locator=ticker.LogLocator(), cmap=cm.PuBu_r, levels=LLLLL);plot1.ylabel(r'$p/bar$'); plot1.xlabel(r'$T$/mK');
DensityPlot = plot1.pcolormesh(X*(10**3), Y, EnergyDensity_fA/deltaB_WeakCoupling_ZeroTemperature);plot1.ylabel(r'$p/bar$'); plot1.xlabel(r'$T$/mK')
cb = plot1.colorbar(DensityPlot)
# plot1.clim(0, 0) # restrict the upper limit of colorbar
# cb.set_ticks([10.0, 30.0, 50.0, 70.0, 90.0, 110.0, 130.0, 150.0, 170.0, 190.0, 210.0, 230.0, 250.0]) # set the position of ticks of colorbar

# contour plot
# LLLLL = np.arange(20, 200, 10)
# cs1 = plot1.contour(X*(10**3), Y, EnergyDensity_fA/deltaB_WeakCoupling_ZeroTemperature, locator=ticker.LogLocator(), levels=LLLLL, colors='black');plot1.clabel(cs1, inline=True, fontsize=9.5, colors='k')
# plot1.ylabel(r'$p/bar$'); plot1.xlabel(r'$T$/mK');
...
plot1.title(r'$\xi_{GL}(p, T)/nm$')
plot1.savefig('Distribution_of_fA_Contour_SCModule_versionIII.pdf');
