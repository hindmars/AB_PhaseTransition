...
for i in range(0, No_loops, 1):
    ...
    lambda_bar_after_normalizationFilter = lambda_bar_array[normalization_filter]

    print(" \n\n lambda bar after normalizaton filter is : ",lambda_bar_after_normalizationFilter)

    net_No_samples += lambda_bar_array.size

    # plot1.matshow(lambda_bar_array)
    # plot1.scatter(np.random.random_sample(array_shape),lambda_bar_array);plot1.ylim([0.0, 1.25])
    plot1.scatter(np.arange(0,lambda_bar_after_normalizationFilter.size,1), lambda_bar_after_normalizationFilter, c = "green");plot1.ylim([0.0, 1.25]);plot1.grid()
