...
stepT = 0.02*(10**-3) 
Temperature = np.arange(2.0*(10**-3), 2.4*(10**-3), stepT) #Kelvin

stepPressure = 0.01*bar
pressure = np.arange(19.0, 22.9*bar, stepPressure)
...
fBGL = np.zeros((lengthPressure,lengthT))
fAGL = np.zeros((lengthPressure,lengthT))
...
#       fBGLRed = alphaRed*3*(delta**2) + 3*beta12345Red*(delta**4)
           ...
           print(" fAGL in SI unit is : ", fAGL_SI)
           print(" fBGL in Si unit is : ", fBGL_SI)
