...
####################################################################################
#########    Small Super Cooling action S_{4} in Enquvist's PRD Eq.3.6    ##########
####################################################################################

def Enqvist_SSC_S4(lambda_pT, lambda_bar):

    f1 = (pi*pi)/(lambda_pT*6.0)
    f2 = lambda_bar/((lambda_bar-1)**(3))

    S4 = f1*f2

    S4_scaled = S4*lambda_pT
...
Temperature = np.asarray(Lotynk.Temperature)*(10**(-3)) # Kelvin
pressure = np.asarray(Lotynk.pressure)
...
# saving array for SSC action S_4 ot Enquvist PRD Eq.3.6
array_S4_SSC = []
array_S4_scaled_SSC = []
...
for iP in range(0, lengthPressure, 1):
    ...
    print(' Temperature is ', Temperature[indexP])
    
    # call the SC_beta_gaps object
    alpha, beta1, beta2, beta3, beta4, beta5, betaA, betaB, DeltaA, DeltaB, phi0, Tcp, xitp, xi0p, t = SC_beta_gaps.calculate_SC_beta_gaps_etc(p,Temperature[indexP])

    t = Temperature[indexP]/Tcp
    ...
    if t >= 1:
       ...
       # array_lambdaBar[indexP] = np.nan
       array_lambdaBar.append(np.nan)
       array_S4_SSC.append(np.nan)
       array_S4_scaled_SSC.append(np.nan)
      ...
      # lambda
        ...
        print(" \n lambda_pT is ", lambda_pT)

      # lambda_bar
        lambda_bar = (9/2)*lambda_pT*(M2/(delta_pT*delta_pT))
        # array_lambdaBar[indexP] = lambda_bar
        array_lambdaBar.append(lambda_bar)
        print(" \n lambda_bar is ", lambda_bar)

      # Enqvist's SSC S_4

        S4, S4_scaled = Enqvist_SSC_S4(lambda_pT, lambda_bar)
      
        array_S4_SSC.append(S4)
        print(" \n SSC_S4 is ", S4)

        array_S4_scaled_SSC.append(S4_scaled)
        print(" \n SSC_S4_scaled is ", S4_scaled)
