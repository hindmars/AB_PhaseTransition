...
def plot_wall(A, pot, gr, sigma_AB):
...
def get_and_plot_wall(t,p):
    ...
    A, pot, gr, sigma_AB = get_wall(t, p)
    # A, pot, gr = hw.krylov_bubble(h.alpha_norm(t), h.beta_norm_asarray(t,p), gr_pars=(500,125), dim=1)
    
    # Plotting
    ax = plot_wall(A, pot, gr, sigma_AB)

    return (A,pot,gr), sigma_AB, ax



Apg_kry, sigma_AB, ax = get_and_plot_wall(t,p)
