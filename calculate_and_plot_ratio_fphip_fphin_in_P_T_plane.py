...
########################################################################################################
###########    plot ratio fphi_n/abs(fphi_p) with index_p, fucntion of lambda_bar       ################
########################################################################################################

pl1 = plot1.plot(array_lambdaBar[1650,:], array_ratio_fphin_fphip[1650,:],'r-x',array_lambdaBar[1300,:], array_ratio_fphin_fphip[1300,:],'b-v',array_lambdaBar[1080,:], array_ratio_fphin_fphip[1080,:],'g-s',array_lambdaBar[1030,:], array_ratio_fphin_fphip[1030,:],'m-o');plot1.ylabel(r'$f(\phi_{-})/|f(\phi_{+})|$'); plot1.xlabel(r'$\bar{\lambda}$')

plot1.legend(['p = 33bar','p = 26bar','p =21.6bar','p = 20.6bar'],loc = 'upper left')

# add title for the plot
plot1.title(r'$f(\phi_{-})/|f(\phi_{+})|$ vs. $\bar{\lambda}$')

plot1.savefig('ratio_fphi_n_fphi_p_AB_lambdaBar_plot_Phase_Enqvist_model_1.pdf');
